...
export default class ScannerDoc extends PureComponent {
  static propTypes = {
  ...
  componentDidMount() {
    ...
    // //This code below would unmount camera when done to prevent black screen on navigating back
  ...
  componentWillUnmount() {
    ...
    // this.setState({takingPicture: false, processingImage: false}); 
  ...
  onPictureTaken = (event) => {
    ...
    //console.log("picture state"+ this.state.takingPicture)
  }
  ...
  onPictureProcessed = (event) => {
    ...
    AsyncStorage.setItem('scannedImage', event.croppedImage);
    console.log("scanned Image1: "+ event.croppedImage);
    this.props.navigation.navigate('CropperPage');
    this.setState({
      takingPicture: false,
      processingImage: false,
      ...
      <>
        <View style={styles.buttonBottomContainer}>
          <View style={styles.buttonGroup}>
            <TouchableOpacity
              style={styles.button}
              // replaced oncancel with navigate back
          ...
          <View>
            {/* change false value from 16 to 0 */}
            <View style={[styles.buttonActionGroup, { justifyContent: 'flex-end', marginBottom: this.props.hideSkip ? 0 : 0 }]}> 
              <ScannerFilters
                filterId={this.state.filterId}
                onFilterIdChange={this.onFilterIdChange}
              />
      ...
      <View style={styles.cameraNotAvailableContainer}>
        ...
        <View style={styles.buttonBottomContainer}>
          <View style={styles.buttonGroup}>
            <TouchableOpacity
              style={styles.button}
              // changed oncancel with navigate back
              onPress={()=>this.props.navigation.goBack()}
              activeOpacity={0.8}
            >
              <Icon name="ios-close-circle" size={40} style={styles.buttonIcon} />
