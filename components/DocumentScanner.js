...
export default class DocumentScanner extends PureComponent {
  ...
  // The picture was taken and cached. You can now go on to using it.
    ...
    //console.log('scanned Image : '+ event.croppedImage);
    
        AsyncStorage.getItem('scannedImages').then((scannedImages) => {
        const scan = scannedImages ? JSON.parse(scannedImages) : [];
        scan.push(event.croppedImage);
        AsyncStorage.setItem('scannedImages', JSON.stringify(scan));
        console.log("scanned Image: "+scan)
    }); 
              ...
              {this.props.hideSkip ? null : (
                <View style={[styles.buttonGroup, {marginLeft: 8}]}>
                  <TouchableOpacity
                    ...
                    />
                    <Text style={[styles.buttonText,{display:'none'}]}>{''}</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
      ...
      <>
        <View style={styles.buttonBottomContainer}>
          ...
          <View>
            ...
            <View style={[styles.buttonGroup],{display:'none'}}>
              {this.props.hideSkip ? null : (
                <TouchableOpacity
                  style={[styles.button, disabledStyle]}
                  onPress={cameraIsDisabled ? () => null : this.props.onSkip}
